...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: DaytraderCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: DaytraderCoind.pid)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. DaytraderCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. DaytraderCoin is probably already running."), strDataDir));
    ...
    /* Denominations
       ...
       For example:
       1DTC+1000 == (.1DTC+100)*10
       10DTC+10000 == (1DTC+1000)*10
    */
