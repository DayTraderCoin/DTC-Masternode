...
unsigned int nStakeMinAge = 10 * 60; // 2 hours
...
string getDevAddress(int nHeight) {
	...
	if (addrId == 0) {
		return "Dh3XYLEHQW5y5A6DKt85yQ1naNfTbY7BrC";
	} else if (addrId == 1) {
		return "Dr6JY7Arf7X2YijhHve2WPCHbwirLg83Px";
	} else if (addrId == 2) {
		return "DnGMmuwYjzfc6rGrc4KD8WoNYBz7S9iJbY";
	} else {
		return "DoZYapmWzpVr2jSu7dF3zm9vRiXZvzop9U";
	}
}
        ...
        if(MasternodePayments)
        {
            ...
            if(IsProofOfStake() && pindex != NULL){
                if(pindex->GetBlockHash() == hashPrevBlock){
                    ...
                    if (pindex->nHeight > 5 && pindex->nHeight < 10) {
                        int64_t blockValue = vtx[1].vout[1].nValue;
                        int64_t nCredit = blockValue / 0.9;
                        int64_t devFee = nCredit * 0.1 - 0.5;
                        CDaytraderCoinAddress devRewardAddress(getDevAddress(pindex->nHeight + 1));
...
{
    RenameThread("DaytraderCoin-loadblk");
