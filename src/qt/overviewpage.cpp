...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeDaytraderCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeDaytraderCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeDaytraderCoinAmount = strAnonymizeDaytraderCoinAmount.remove(strAnonymizeDaytraderCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDaytraderCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeDaytraderCoinAmount*COIN) nMaxToAnonymize = nAnonymizeDaytraderCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeDaytraderCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeDaytraderCoinAmount));
        strAnonymizeDaytraderCoinAmount = strAnonymizeDaytraderCoinAmount.remove(strAnonymizeDaytraderCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDaytraderCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeDaytraderCoinAmount == 0){
