...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    WSADATA wsadata;
    int ret = WDTCtartup(MAKEWORD(2,2), &wsadata);
    if (ret != NO_ERROR)
    {
        strError = strprintf("Error: TCP/IP socket library failed to start (WDTCtartup returned error %d)", ret);
        LogPrintf("%s\n", strError);
        return false;
    }
#endif
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. DaytraderCoin is probably already running."), addrBind.ToString());
        else
